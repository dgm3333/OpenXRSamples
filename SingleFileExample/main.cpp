...
// This example implements a simple XR session.
// See: https://registry.khronos.org/OpenXR/specs/1.0/html/xrspec.html#session
...
XrSessionState xr_session_state = XR_SESSION_STATE_UNKNOWN;								// https://registry.khronos.org/OpenXR/specs/1.0/man/html/XrSessionState.html
...
Result openxr_init(const char* app_name, int64_t swapchain_format) {
		...
		}))
	{
		...
		if (result != XR_SUCCESS) {
			return { "Graphics requirements failed with code: " + std::to_string(result) + ". To pass this point your headset must be running and desktop link active", false };
		}
		if (!d3d_init(requirement.adapterLuid))
			return { "Graphics requirements failed. To pass this point your headset must be running and desktop link active", false };
...
// Do not use - for overview and navigation only...
void programFlowDummy() {
	...
	openxr_init(app_name, swapchain_format);		// initialise the XR session
	xrCreateInstance(&createInfo, &xr_instance);
	xrGetSystem(xr_instance, &systemInfo, &xr_system_id);
	xrCreateSession(xr_instance, &sessionInfo, &xr_session);
	xrCreateReferenceSpace(xr_session, &ref_space, &xr_app_space);  // XR_REFERENCE_SPACE_TYPE_LOCAL = seated vs XR_REFERENCE_SPACE_TYPE_STAGE = centre of room


	openxr_make_actions();		// create actions handlers (eg pose and button watchers)
	...
	while (!quit) {
		...
		if (xr_running) {
			openxr_poll_actions();			// polls the action handlers created in openxr_make_actions
			xrSyncActions(xr_session, &sync_info);
			...
			for (uint32_t hand = 0; hand < 2; hand++) {
				...
				// If we have a select event, update the hand pose to match the event's timestamp
				if (xr_input.handSelect[hand])
					xrLocateSpace(xr_input.handSpace[hand], xr_app_space, select_state.lastChangeTime, &space_location);
			}

			app_update();					// adds a cube when select button pressed

			openxr_render_frame();
			xrWaitFrame(xr_session, nullptr, &frame_state);									// Block until previous frame displayed and predict when next one will be rendered
			xrBeginFrame(xr_session, nullptr);												// initialise frame
			openxr_poll_predicted(frame_state.predictedDisplayTime);						// Update hand positions based on the predicted time frame will be rendered
			for (uint32_t hand = 0; hand < 2; hand++)
				xrLocateSpace(xr_input.handSpace[hand], xr_app_space, predicted_time, &spaceRelation);
			app_update_predicted();															// update the hand cubes to reflect the hand positions
			openxr_render_layer(frame_state.predictedDisplayTime, views, layer_proj);		// render layer in compositor
			d3d_render_layer(view, surface);
			app_draw(view);

			xrEndFrame(xr_session, &end_info);												// send layer to display
